...
app.post('/api/auth/register', async (req, res) => {
  ...
  const result = db.prepare('INSERT INTO users (name, email, password, avatar_color) VALUES (?, ?, ?, ?)').run(name, email.toLowerCase(), hash, avatarColor || '#6366f1');
...
app.get('/api/pluggy/connect-token', auth, async (req, res) => {
  try {
    const token = await pluggy.createConnectToken({ userId: req.user.id, itemId: req.query.itemId || null });
    res.json({ token });
...
app.post('/api/pluggy/item', auth, async (req, res) => {
  ...
  try {
    ...
    let dbItemId;
    if (existing) {
      db.prepare("UPDATE pluggy_items SET status = ?, last_sync = datetime('now') WHERE item_id = ?").run(item.status, itemId);
      dbItemId = existing.id;
    } else {
      const result = db.prepare('INSERT INTO pluggy_items (user_id, item_id, institution_name, institution_id, status) VALUES (?, ?, ?, ?, ?)').run(req.user.id, itemId, conn.name || 'Banco', conn.id || null, item.status);
    ...
    res.json({ success: true, bank: conn.name, newTransactions: syncResult.txnCount, error: syncResult.error });
  } catch (err) { res.status(500).json({ error: err.message }); }
});

app.get('/api/pluggy/items', auth, (req, res) => {
  const items = db.prepare("SELECT pi.*, (SELECT COUNT(*) FROM bank_accounts ba WHERE ba.item_id = pi.id) as account_count FROM pluggy_items pi WHERE pi.user_id = ? ORDER BY pi.institution_name").all(req.user.id);
  res.json(items);
});
...
app.delete('/api/pluggy/items/:id', auth, async (req, res) => {
  ...
  try { await pluggy.deleteItem(item.item_id); } catch (e) { console.warn('Erro ao deletar:', e.message); }
...
app.get('/api/accounts', auth, (req, res) => {
  const accounts = db.prepare("SELECT ba.*, pi.institution_name, pi.status as sync_status FROM bank_accounts ba LEFT JOIN pluggy_items pi ON pi.id = ba.item_id WHERE ba.user_id = ? ORDER BY ba.type, ba.name").all(req.user.id);
  res.json(accounts);
});
...
app.post('/api/accounts', auth, (req, res) => {
  ...
  const result = db.prepare("INSERT INTO bank_accounts (user_id, name, type, subtype, number, balance, color, manual) VALUES (?, ?, ?, ?, ?, ?, ?, 1)").run(req.user.id, bank, type || 'BANK', subtype || 'CHECKING', number || '****', balance || 0, color || '#6366f1');
...
app.post('/api/transactions', auth, (req, res) => {
  ...
  const result = db.prepare("INSERT INTO transactions (user_id, description, amount, type, category, source, date, account_name, card_name, imported_from) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, 'manual')").run(req.user.id, description, Math.abs(amount), type, category || 'Outros', source, date || new Date().toISOString().split('T')[0], account_name || null, card_name || null);
...
app.get('/api/dashboard', auth, (req, res) => {
  ...
  const totalBalance = db.prepare("SELECT COALESCE(SUM(balance),0) as total FROM bank_accounts WHERE user_id=?").get(uid).total;
  const byCategory = db.prepare("SELECT category, SUM(amount) as total, source, COUNT(*) as count FROM transactions WHERE user_id=? AND type IN ('despesa','compra') AND date BETWEEN ? AND ? GROUP BY category, source ORDER BY total DESC").all(uid, from, to);
  const monthlyEvolution = db.prepare("SELECT strftime('%Y-%m', date) as month, SUM(CASE WHEN type='receita' THEN amount ELSE 0 END) as income, SUM(CASE WHEN type='despesa' THEN amount ELSE 0 END) as bank_expense, SUM(CASE WHEN type='compra' THEN amount ELSE 0 END) as cc_spend FROM transactions WHERE user_id=? AND date >= ? GROUP BY month ORDER BY month").all(uid, daysAgo(180));
  const lastSync = db.prepare("SELECT last_sync FROM user_settings WHERE user_id=?").get(uid)?.last_sync;
  res.json({ summary: { bankIncome, bankExpense, ccSpend, ccPayment, totalBalance, savingRate: bankIncome > 0 ? ((bankIncome - bankExpense - ccSpend) / bankIncome * 100).toFixed(1) : 0 }, byCategory, monthlyEvolution, lastSync });
});

app.get('/api/sync/log', auth, (req, res) => {
  try {
    const logs = db.prepare("SELECT sl.*, pi.institution_name FROM sync_log sl JOIN pluggy_items pi ON pi.id = sl.item_id WHERE sl.user_id = ? ORDER BY sl.created_at DESC LIMIT 20").all(req.user.id);
    res.json(logs);
  } catch(e) { res.json([]); }
});
...
app.patch('/api/settings', auth, (req, res) => {
  ...
  db.prepare("INSERT INTO user_settings (user_id, theme, login_bg, notif_budget, notif_weekly, notif_big_txn, sync_daily, sync_on_open) VALUES (?, ?, ?, ?, ?, ?, ?, ?) ON CONFLICT(user_id) DO UPDATE SET theme=excluded.theme, login_bg=excluded.login_bg, notif_budget=excluded.notif_budget, notif_weekly=excluded.notif_weekly, notif_big_txn=excluded.notif_big_txn, sync_daily=excluded.sync_daily, sync_on_open=excluded.sync_on_open").run(req.user.id, theme||0, login_bg||0, notif_budget?1:0, notif_weekly?1:0, notif_big_txn?1:0, sync_daily?1:0, sync_on_open?1:0);
  res.json({ success: true });
});
...
app.post('/api/webhook', (req, res) => {
  ...
  try {
    ...
    if (event === 'transactions/created' || event === 'transactions/updated') {
      ...
      if (item) {
        syncItem(item, item.user_id).catch(e => console.error('[WEBHOOK] sync error:', e.message));
      }
    }
...
app.get('/api/health', (req, res) => {
  res.json({ status: 'ok', version: '4.1.0', timestamp: new Date().toISOString(), pluggy: !!process.env.PLUGGY_CLIENT_ID ? 'configured' : 'not configured' });
});
...
// ─── INICIAR SERVIDOR ─────────────────────────────────────
app.listen(PORT, () => {
  console.log(`FinanceOS Pro Backend v4.1 rodando na porta ${PORT}`);
  startCronJobs();
});
